...
def set_rate_limiting(rate_limit, min_wait=timedelta(milliseconds=50)):
  ...
  ValueError still might be raised.
...
def geosearch(latitude, longitude, title=None, results=10, radius=1000):
  ...
  * radius - Search radius in meters. The value must be between 10 and 10000
  ...
  if 'error' in raw_results:
    if raw_results['error']['info'] in ('HTTP request timed out.', 'Pool queue is full'):
      raise ValueError('{0}|{1}'.format(latitude, longitude))
    else:
      raise ValueError(raw_results['error']['info'])
...
def summary(title, sentences=0, chars=0, auto_suggest=True, redirect=True):
  ...
  * redirect - allow redirection without raising ValueError
  '''

  # use auto_suggest and redirect to get the correct article
  # also, use page's error checking to raise ValueError if necessary
  ...
  def __load(self, redirect=True, preload=False):
    ...
    if 'missing' in page:
      if hasattr(self, 'title'):
        ...
        raise ValueError(pageid=self.pageid)
    ...
    # whatever silly reason
      ...
      if redirect:
        ...
        if 'normalized' in query:
          ...
          assert normalized['from'] == self.title, ValueError
        ...
        assert redirects['from'] == from_title, ValueError

        # change the title and reload the whole object
        self.__init__(redirects['to'], redirect=redirect, preload=preload)

      else:
        raise ValueError(getattr(self, 'title', page['title']))
    ...
    # then the page must be a disambiguation page
      ...
      raise ValueError(getattr(self, 'title', page['title']), may_refer_to)
